...
CFIndex PSDatasetApodizeRamLakMinimumNumberOfDimensions()
{
    return 1;
}
...
CFMutableDictionaryRef PSDatasetApodizeRamLakCreateDefaultFunctionParametersForDataset(PSDatasetRef theDataset, CFErrorRef *error)
{
    ...
    CFArrayAppendValue(parametersNames, kPSDatasetApodizeRamLakCutoff);
...
{
    if(index==0) return kPSDatasetApodizeRamLakCutoff;
    return NULL;
}

static PSDependentVariableRef PSDatasetApodizeRamLakCreateBlock(PSDatasetRef theDataset, CFMutableDictionaryRef functionParameters, CFErrorRef *error)
{
    if(error) if(*error) return NULL;
    if(!CFDictionaryContainsKey(functionParameters, kPSDatasetApodizationFunctionValues)) return false;
    CFMutableArrayRef parametersValues = (CFMutableArrayRef) CFDictionaryGetValue(functionParameters, kPSDatasetApodizationFunctionValues);
    ...
    dispatch_apply(horizontalDimensionNpts, queue,
                   ^(size_t coordinateIndex) {
                       ...
                       if(horizontalCoordinate <=cutoff) function1D[coordinateIndex] = horizontalCoordinate;
                   });
    free(coordinates);
...
PSDatasetRef PSDatasetApodizeRamLakCreateByApodizing(PSDatasetRef theDataset, CFMutableDictionaryRef functionParameters, CFErrorRef *error)
{
    if(error) if(*error) return NULL;
    if(!CFDictionaryContainsKey(functionParameters, kPSDatasetApodizationFunctionValues)) return false;
    PSDependentVariableRef theBlock = PSDatasetApodizeRamLakCreateBlock(theDataset, functionParameters,error);
